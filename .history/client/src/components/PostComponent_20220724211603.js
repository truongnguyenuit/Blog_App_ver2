...
const PostComponent = ({ title, description, img, user, time }) => {
  return (
    <div className="w-[430px] mx-6 my-2 flex flex-col bg-white drop-shadow-md ">
      <div className='flex'>
        <div className="bg-white p-1 rounded-[100px]">
          <img
            className="w-[60px] h-[60px] object-cover rounded-[100px]"
            src={user.img}
            alt=""
          />
        </div>
        <div>
          <p className='fw-bold'>{user.realname}</p>
          <p>{user.email}</p>
        </div>
      </div>
