...
const PostComponent = ({ atitle, adescription, aimg, user, time, a_id, page }) => {
  ...
  const [isUpdateModalVisible, setIsModalVisibl1] = useState(false);

  const showModal1 = () => {
    setIsModalVisibl1(true);
  };
  const handleOk1 = () => {
    setIsModalVisibl1(false);
  };
  const handleCancel1 = () => {
    setIsModalVisibl1(false);
  };
  ...
  return (
    <div className="w-[430px] mx-6 my-2 flex flex-col bg-white drop-shadow-md rounded-[8px] overflow-hidden">
      ...
      <Modal title="Update Blog" visible={isUpdateModalVisible} onOk={upBlog} onCancel={handleCancel1}>
